...
# Load the data (assuming 'Obesity_data.csv' contains the dataset)
...
# Define model with uninformative prior (using only 4 predictors)
mod1 <- textConnection("model{
  ...
  for (j in 1:5) {
    beta[j] ~ dnorm(0, 0.01)  # Normal prior with mean 0 and precision 0.01
  }
}")

# Prepare data for JAGS model
n <- length(Y)  # Define number of observations
data_test1 <- list(Y = Y, X = X, n = n)  # Pass the entire scaled matrix X to JAGS
...
# Fit the uninformative model
model1 <- jags.model(mod1, data = data_test1, n.chains = 2, inits = inits)
update(model1, 500, progress.bar = "none")  # Burn-in phase

# Sample from the posterior distribution
samps1 <- coda.samples(model1, variable.names = c("beta"), n.iter = 2000, thin = 2)
...
# Compute DIC for Uninformative Model
DIC_uninformative <- dic.samples(model1, n.iter = 1000, n.thin = 5)
DIC_uninformative
